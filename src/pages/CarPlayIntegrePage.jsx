  ...
  return (
    <div className="grain min-h-screen">
      ...
      <section className="relative h-[50vh] min-h-[360px] flex items-end overflow-hidden">
        <img src={images.integre1} alt="CarPlay intégré" className="img-cover absolute inset-0" loading="eager" />
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-5 md:px-8 pb-12 w-full">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="text-[11px] tracking-[4px] uppercase font-semibold" style={{ color: '#d4a855' }}>Remplacement autoradio</p>
            </div>
      ...
      <section className="py-20 px-5 md:px-8">
        <div className="max-w-[1400px] mx-auto">
          ...
          <p className="text-center text-xs mt-4 font-body" style={{ color: '#d4a855' }}>▶ Cliquez sur lecture pour voir le tutoriel d'installation</p>
        </div>
      </section>
      ...
      <section className="py-20 px-5 md:px-8 border-y border-white/[0.04]"
        ...
        <div className="max-w-[900px] mx-auto">
          <SectionTitle subtitle="Compatibilité" title="VOTRE MODÈLE EST LISTÉ ?" accentColor="#d4a855" />
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
            {['BMW Série 3 E90', 'VW Golf 7', 'Mercedes W204', 'Audi A3 / A4', 'Peugeot 308 / 3008', 'Renault Clio / Megane', 'Ford Focus / Fiesta', 'Toyota / Honda'].map((car, i) => (
              <motion.div key={i} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }}
                ...
                style={{ background: 'rgba(212,168,85,0.03)', borderColor: 'rgba(212,168,85,0.12)' }}>
                <p className="font-body text-sm text-brand-text">{car}</p>
              </motion.div>
            ))}
          </div>

          {/* CTA "pas votre modèle" */}
          <motion.div
            initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }} transition={{ delay: 0.4 }}
            className="mt-10 rounded-2xl p-6 text-center border"
            style={{ background: 'rgba(212,168,85,0.04)', borderColor: 'rgba(212,168,85,0.16)' }}
          >
            <p className="font-body text-brand-muted text-sm mb-1">Vous ne trouvez pas votre modèle ?</p>
            <p className="font-body text-white/80 text-[15px] font-semibold mb-5">
              Contactez-nous — on pourra peut-être vous le trouver.
            </p>
            <Link to="/contact">
              <motion.button
                whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                className="px-8 py-3.5 rounded-xl font-bold text-[14px] text-black relative overflow-hidden"
                style={{ background: 'linear-gradient(135deg, #d4a855, #f0cc7a 50%, #d4a855)', backgroundSize: '200% auto', boxShadow: '0 4px 24px rgba(212,168,85,0.22)' }}
              >
      ...
      <section className="py-20 px-5 md:px-8 text-center">
        <p className="font-body text-brand-muted mb-6">Besoin d'aide pour choisir ou faire installer ?</p>
        <div className="flex flex-wrap justify-center gap-4">
          <Link to="/installation">
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
              ...
              <span className="absolute inset-0" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.15) 0%, transparent 60%)' }} />
              <span className="relative z-10">Réserver l'installation</span>
            </motion.button>
          </Link>
          <Link to="/contact">
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
              className="px-10 py-4 rounded-2xl font-semibold text-[15px] border transition-all"
              style={{ color: '#d4a855', borderColor: 'rgba(212,168,85,0.30)', background: 'rgba(212,168,85,0.06)' }}>
              Nous contacter
            </motion.button>
          </Link>
        </div>
